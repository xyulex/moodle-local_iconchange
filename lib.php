...
class IconChange{
    ...
    public static function create_table_data($activities) {
        ...
        foreach ($activities as $a) {
            ...
            $imgtag = html_writer::img($currentimage, null, $imgattributes);

            $data[] = [$a, $imgtag, $options];
        }
    ...
    public static function print_table_data($data) {
        ...
        $table->head = [mb_strtoupper(get_string('activity')),
            mb_strtoupper(get_string('icon')), mb_strtoupper(get_string('actions'))];
...
function local_iconchange_extend_navigation($navigation) {
    ...
    if (has_capability('moodle/site:config', $systemcontext)) {
        $node = $navigation->add(get_string('changeicons', 'local_iconchange'), new moodle_url('/local/iconchange/list_icon.php'),
        navigation_node::TYPE_CONTAINER, null, null, new pix_icon('i/edit', ''));
